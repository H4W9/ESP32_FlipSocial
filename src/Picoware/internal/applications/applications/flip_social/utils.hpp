...
// Direct-tap touch keyboard (defined in the sketch's TouchKeyboard.cpp). Used by
// the user / password / post views instead of the button-driven Keyboard widget.
bool touchKeyboardInput(TFT_eSPI &tft, uint16_t fg, uint16_t bg, char *buffer,
                        size_t bufLen, const char *title, bool password);
